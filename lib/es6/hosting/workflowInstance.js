...
WorkflowInstance.prototype.create = async function(execContext, workflowVersion, methodName, args, lockInfo) {
    ...
    try {
        if (self._engine.isIdle(await self._engine.invoke())) {
...
WorkflowInstance.prototype._enterLockForCreatedInstance = async function(lockInfo) {
    if (lockInfo) {
        await this._host._enterLockForCreatedInstance(this, lockInfo);
    }
};
